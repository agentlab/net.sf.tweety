...
public abstract class AbstractDeductiveArgumentationReasoner
		implements Reasoner<PropositionalFormula, PropositionalFormula> {
	 ...
	 * 
	 * @param beliefBase
	 *            some belief base (must be of class DeductiveKnowledgebase).
	 * @param categorizer
	 *            some categorizer.
	 * @param accumulator
	 *            some accumulator.
	...
	public AbstractDeductiveArgumentationReasoner(Categorizer categorizer, Accumulator accumulator) {
		...
		// throw new IllegalArgumentException("Knowledge base of class
		// DeductiveKnowledgebase expected.");
	 ...
	 */
	protected abstract ArgumentTree getArgumentTree(BeliefBase<PropositionalFormula> beliefBase, DeductiveArgument arg);
	...
	public Answer query(BeliefBase<PropositionalFormula> beliefBase, PropositionalFormula query) {
		log.trace("Querying " + beliefBase + " with " + query);
		DeductiveKnowledgeBase kb = (DeductiveKnowledgeBase) beliefBase;
		// 1.) get all arguments for the query
		Set<DeductiveArgument> proArguments = kb.getDeductiveArguments(query);
		// 2.) get all arguments for the negation of the query
		Set<DeductiveArgument> conArguments = kb.getDeductiveArguments(new Negation(query));
		...
		for (DeductiveArgument arg : proArguments)
			proTrees.add(this.getArgumentTree(beliefBase, arg));
		...
		for (DeductiveArgument arg : conArguments)
			conTrees.add(this.getArgumentTree(beliefBase, arg));
