...
//	private ClBeliefSet beliefBase;
	 ...
	 */
	private boolean humanFriendly = true;
...
//	}
	 ...
	 */
	public RankingFunction getSemantic(BeliefBase<Conditional> beliefBase) {
		if(rfunc == null) { 
			if(!isComplete()) {
				if(!process(beliefBase)) {
					return null;
				}
			}
			
			rfunc = new RankingFunction((PropositionalSignature) beliefBase.getSignature());
	 ...
	 */
	public boolean process(BeliefBase<Conditional> beliefBase) {
		prepare(beliefBase);
