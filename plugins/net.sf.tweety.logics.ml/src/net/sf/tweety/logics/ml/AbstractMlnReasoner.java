 ...
 */
public abstract class AbstractMlnReasoner implements Reasoner<MlnFormula, FolFormula> {
...
//		this(beliefBase, (FolSignature) beliefBase.getSignature());
	 ...
	 *            a Markov logic network.
	 * @param signature
	 *            another signature (if the probability distribution should be
	 *            defined on that one (that one should subsume the signature of
	 *            the Markov logic network)
	 */
	public AbstractMlnReasoner(FolSignature signature) {
//		super(beliefBase);
...
//					"Given signature is not a super-signature of the belief base's signature.");
	 ...
	 */
	@Override
	public Answer query(BeliefBase<MlnFormula> beliefBase, FolFormula query) {
		if (!(query.isGround()))
			throw new IllegalArgumentException(
					"Reasoning in Markov logic with naive MLN reasoner is only defined for ground FOL formulas.");
		double result = this.doQuery(beliefBase, query);
	 ...
	 *         signature, that are satisfied in the given Herbrand
	 *         interpretation.
	 */
	protected int numberOfGroundSatisfactions(FolFormula formula, HerbrandInterpretation hInt) {
		...
		for (RelationalFormula f : formula.allGroundInstances(this.getSignature().getConstants()))
			if (hInt.satisfies((FolFormula) f))
				num++;
		return num;
	}
	 ...
	 * 
	 * @param query
	 *            a fol formula guaranteed to be ground.
	 * @return the answer of the query.
	 */
	protected abstract double doQuery(BeliefBase<MlnFormula> beliefBase, FolFormula query);
}
