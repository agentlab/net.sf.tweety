...
/**
 * Wrapper for the PossibleWorld providing better representation mechanisms, it
 * knows all the possible propositions (the signature) and therefore provides a
 * complete representation that maps a boolean to the Proposition.
 * 
 * It also provides ordering of the (proposition, boolean) pairs in the enhanced
 * representation. The default behavior is an alphabetically ordering of the
 * Propositions but it can be changed by the user.
 * 
 * Internally it uses the smaller and therefore faster implementation of
 * PossibleWorld for satisfication test
 * 
 * @author Tim Janus
 */
public class NicePossibleWorld extends InterpretationSet<PropositionalFormula>
		implements Comparator<Pair<Proposition, Boolean>> {

	/**
	 * Implementation of possible world with a small memory print and fast
	 * satisfication tests.
	 */
	...
	/**
	 * Ctor: Generates the possible world using the given set of true
	 * propositions and generated a representation of the possible world using
	 * the given signature. The default alphabetically sort behavior is used to
	 * order the representation.
	 * 
	 * @param trueInWorld
	 *            A set of propositions describing those propositions that are
	 *            true in this possible world. It has to be a subset of
	 *            signature.
	 * @param signature
	 *            A set of propositions describing all know propositions by the
	 *            the problem, the signature. It has to be a super set of
	 *            trueInWorld
	 */
	public NicePossibleWorld(Collection<PropositionalFormula> trueInWorld, Collection<Proposition> signature) {
	 ...
	 * Ctor: Generates the possible world using the given set of true
	 * propositions and generated a representation of the possible world using
	 * the given signature. The sort behavior given in comparator is used to
	 * order the representation.
	 * 
	 * @param trueInWorld
	 *            A set of propositions describing those propositions that are
	...
	public NicePossibleWorld(Collection<PropositionalFormula> trueInWorld, Collection<Proposition> signature,
		...
		trueInWorld.stream().filter(p -> p instanceof Proposition).map(p -> (Proposition) p)
				.forEach(p -> this.representation.add(new Pair<Proposition, Boolean>(p, true)));
		// for (PropositionalFormula p : trueInWorld) {
	...
	/**
	 * Changes the signature of the world. That means the set of propositions
	 * has changed. The new set might contain new propositions and delete old
	 * propositions. The representation data structure is updated to reflect the
	 * new signature. There is a border case if a Proposition is deleted that is
	 * true in this world. Then the world is not representable with the given
	 * signature anymore, if this is the case then the representation data
	 * structure is cleared.
	 * 
	 * @param signature
	 *            A collection of propositions defining the new signature.
	 * @return true if representation is successfully updated and false if the
	 *         world is not representable by the given signature anymore.
	 */
	public boolean setSignature(Collection<Proposition> signature) {
		...
		// Fourth: Update the representation Collection, it sorts automatically
	...
	/**
	 * Changes the used comparator to sort the representation structure and
	 * resorts the representation. The default behavior is an alphabetically
	 * ordering. The default behavior is selected if null is given as argument
	 * or if an instance of the NicePossibleWorld is given as argument.
	 * 
	 * @param comparator
	 *            The new comparator used to sort the representation. If a null
	 *            is given the default alphabetically ordering is used.
	 */
	...
	/**
	 * @return The PossibleWorld instance that is used for satification tests
	 *         etc.
	 */
	...
	// @Override
	// public boolean satisfies(BeliefBase<Proposition> beliefBase)
